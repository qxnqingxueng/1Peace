    ...
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0B132B] px-6 py-12 font-sans text-white">
      ...
      <div className="relative z-10 w-full max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          {cards.map(({ id, title, subtitle, description, Scene, cardTone }, index) => (
            <motion.button
              key={id}
              type="button"
              ...
              onClick={() => onLaunch?.(id)}
              className={`group flex h-[34rem] w-full flex-col rounded-3xl border border-[#475569] bg-white/[0.05] p-5 text-left shadow-[0_18px_56px_rgba(2,9,19,0.45)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 ${cardTone}`}
            >
              <Scene />
              <div className="mt-5 flex flex-1 flex-col">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/65">{title}</p>
                <h2 className="mt-2 text-3xl font-semibold">{subtitle}</h2>
