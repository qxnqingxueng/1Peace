import type { MalaysiaNewsResponse, NewsArticle, NewsCategory } from '../types.js';
...
let fnCache: { data: MalaysiaNewsResponse; ts: number } | null = null;
...
function extractRssImage(block: string, description: string, content: string): string | null {
  const candidates = [
...
function classifyCategory(text: string, defaultCategory: NewsCategory): NewsCategory {
...
async function fetchFeed(feed: (typeof FEEDS)[number]): Promise<NewsArticle[]> {
  ...
  if (!res.ok) throw new Error(`Feed ${res.status}: ${feed.url}`);
  const xml = await res.text();
  const blocks = xml.match(/<item\b[\s\S]*?<\/item>/gi) ?? [];

  return blocks.flatMap((block): NewsArticle[] => {
...
async function enrichImages(articles: NewsArticle[]): Promise<NewsArticle[]> {
...
function buildFallbackSections(generatedAt: string): MalaysiaNewsResponse['sections'] {
  const make = (category: NewsCategory) => ({
    featured: {
...
export async function fetchMalaysiaNews(): Promise<MalaysiaNewsResponse> {
  if (fnCache && Date.now() - fnCache.ts < FN_CACHE_TTL_MS) return fnCache.data;
  ...
  const capped = (Object.keys(fallbackSections) as NewsCategory[]).flatMap((cat) =>
    articles.filter((a) => a.category === cat).slice(0, MAX_ARTICLES_PER_CATEGORY),
  );
  ...
  const sections = (Object.keys(fallbackSections) as NewsCategory[]).reduce(
    (acc, cat) => {
    ...
    {} as MalaysiaNewsResponse['sections'],
  );

  const result: MalaysiaNewsResponse = { generatedAt, timezone: TIMEZONE, source: 'live', sections };
